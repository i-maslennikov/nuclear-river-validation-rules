...
namespace NuClear.CustomerIntelligence.Domain.Model.Statistics
{
    public class ProjectStatisticsAggregate : IAggregateRootActor
    {
        ...
        public IReadOnlyCollection<IActor> GetValueObjectActors()
            => new[] { new ReplaceStorageBasedDataObjectsActor<FirmForecast>(_query, _firmForecastBulkRepository, new FirmForecastAccessor()) };
        ...
        private class ProjectStatisticsActor : AggregateRootActorBase<ProjectStatistics>
        {
            public ProjectStatisticsActor(IQuery query, IBulkRepository<ProjectStatistics> projectCategoryStatisticsBulkRepository)
                : base(query, projectCategoryStatisticsBulkRepository, new ProjectStatisticsAccessor())
            {
            }
        }
        ...
        private class ProjectCategoryStatisticsActor : IEntityActor
        {
            ...
            public IReadOnlyCollection<IEvent> ExecuteCommands(IReadOnlyCollection<ICommand> commands)
            {
                var actor = new SyncDataObjectsActor<ProjectCategoryStatistics>(_query, _projectCategoryStatisticsBulkRepository, new ProjectCategoryStatisticsAccessor());
                return actor.ExecuteCommands(commands);
            }

            public IReadOnlyCollection<IActor> GetValueObjectActors()
                => new [] { new ReplaceStorageBasedDataObjectsActor<FirmCategory3>(_query, _firmCategory3BulkRepository, new FirmCategory3Accessor())};
        }
        ...
        private class ProjectStatisticsAccessor : IStorageBasedDataObjectAccessor<ProjectStatistics>
        {
            ...
            public IQueryable<ProjectStatistics> GetSource(IQuery query) => Specs.Map.Facts.ToCI.ProjectStatistics.Map(query);

            public FindSpecification<ProjectStatistics> GetFindSpecification(IReadOnlyCollection<ICommand> commands)
            {
                return new FindSpecification<ProjectStatistics>(x => commands.Cast<IAggregateCommand>().Select(c => c.AggregateId).Distinct().Contains(x.Id));
            }
        }
        ...
        private class ProjectCategoryStatisticsAccessor : IStorageBasedDataObjectAccessor<ProjectCategoryStatistics>
        {
            ...
            public IQueryable<ProjectCategoryStatistics> GetSource(IQuery query) => Specs.Map.Facts.ToCI.ProjectCategoryStatistics.Map(query);

            public FindSpecification<ProjectCategoryStatistics> GetFindSpecification(IReadOnlyCollection<ICommand> commands)
                => Specs.Find.CI.ProjectCategoryStatistics(commands.Cast<IAggregateCommand>().Select(c => c.AggregateId).Distinct().ToArray());
        }
        ...
        private class FirmForecastAccessor : IStorageBasedDataObjectAccessor<FirmForecast>
        {
            ...
            public IQueryable<FirmForecast> GetSource(IQuery query) => Specs.Map.Facts.ToCI.FirmForecast.Map(query);

            public FindSpecification<FirmForecast> GetFindSpecification(IReadOnlyCollection<ICommand> commands)
                => Specs.Find.CI.FirmForecast(commands.Cast<IAggregateCommand>().Select(c => c.AggregateId).Distinct().ToArray());
        }
        ...
        private class FirmCategory3Accessor : IStorageBasedDataObjectAccessor<FirmCategory3>
        {
            ...
            public IQueryable<FirmCategory3> GetSource(IQuery query) => Specs.Map.Facts.ToCI.FirmCategory3.Map(query);

            public FindSpecification<FirmCategory3> GetFindSpecification(IReadOnlyCollection<ICommand> commands)
                => Specs.Find.CI.FirmCategory3(commands.Cast<IAggregateCommand>().Select(c => c.AggregateId).Distinct().ToArray());
        }
    }
}