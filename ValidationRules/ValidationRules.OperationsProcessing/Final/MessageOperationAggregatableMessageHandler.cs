...
{
    public sealed class MessageOperationAggregatableMessageHandler : IMessageProcessingHandler
    {
        private readonly ITelemetryPublisher _telemetryPublisher;
        private readonly ITracer _tracer;

        public MessageOperationAggregatableMessageHandler(ITelemetryPublisher telemetryPublisher, ITracer tracer)
        {
        ...
        private StageResult Handle(Guid bucketId, IEnumerable<IAggregatableMessage> messages)
        {
            try
            {
                foreach (var message in messages.OfType<OperationAggregatableMessage<IOperation>>())
                {
                    ...
                    _telemetryPublisher.Publish<MessageProcessedOperationCountIdentity>(message.Operations.Count);
                    _telemetryPublisher.Publish<MessageProcessingDelayIdentity>((long)(DateTime.UtcNow - message.OperationTime).TotalMilliseconds);
                }
