...
namespace NuClear.ValidationRules.Replication.ConsistencyRules.Facts
{
    public sealed class BranchOfficeOrganizationUnitAccessor : IStorageBasedDataObjectAccessor<BranchOfficeOrganizationUnit>, IDataChangesHandler<BranchOfficeOrganizationUnit>
    {
        ...
        public IQueryable<BranchOfficeOrganizationUnit> GetSource()
            => _query.For<Storage.Model.Erm.BranchOfficeOrganizationUnit>()
                     .Where(x => x.IsActive && !x.IsDeleted)
                     .Select(x => new BranchOfficeOrganizationUnit { Id = x.Id, BranchOfficeId = x.BranchOfficeId });
